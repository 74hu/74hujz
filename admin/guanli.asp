...
%><%=i+(page-1)*PageSize%>.<a href='guanli.asp?sid=<%=sid%>&amp;Action=view&amp;id=<%=ubb(rs("id"))%>'><%=keyname%><%=ubb(rs("username"))%><%if rs("id")=1 then%>站长<%end if%></a><br/>
上次管理:<%=ubb(fordate(rs("lastdate")))%><br/>
<%     
	rs.moveNext
	Next
...
<%end if%>
		...
		Response.Write("<a href='glyh.asp?sid="&sid&"'>[修改帐号]</a><br/>")
end Function
Function add%>
...
Function view
	...
	<card id="index" title="出错啦"><p>ID无效.<br/>
		...
		<%
			...
			Rs.Open Sql,conn,1,1
			if not (rs.bof and rs.eof)  then %>
				用户:<%=ubb(rs("username"))%><br/>
				密码:<%=ubb(rs("password"))%><br/>
				高密:<%=ubb(rs("HU_admin"))%><br/>
                   上次管理:<%=ubb(fordate(rs("lastdate")))%><br/>
                   上次IP:<%=ubb(rs("lastip"))%><br/>
