...
if relid="" or IsNumeric(relid)=False then num=0
if num="" or IsNumeric(num)=False then num=0
pid=request.form("pid")
if pid="" or IsNumeric(pid)=False then Call Error("排序无效！")

br=request.form("br")
if br="" or IsNumeric(br)=False then Call Error("换行无效！")
lx=request.form("lx")
if lx="" or IsNumeric(lx)=False then  Call Error("类型无效！")
	parent=request.form("parent")
	if parent="" or IsNumeric(parent)=False then parent=0
	lid=request.form("lid")
	if lid <>"" then
	if IsNumeric(lid)=False then Call Error("ID无效！")
	end if

	dim reads,fso,filesize
	if url<>"" then
	filename=url
	else
	if Wmlname="" then
	...
	if url="" then rs.addnew
	rs("class")=class1
	rs("pid")=pid
	rs("br")=br
	rs("lx")=lx
	if parent<>0 then rs("parent")=parent
	rs("url")=filename
	rs("active")=active
	rs.update
	rs.close
