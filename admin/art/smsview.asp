...
if rs.eof and rs.bof then
	...
	Counts=rs("smspin")
	Response.Write	"["&noubb(fordate(rs("HU_date")))&"]<br/>"
	Response.Write	noubb(rs("title"))&"<br/>"
	Content=rs("test")
	if request.querystring("o")<>1 then
		...
		ii=clng(request.querystring("ii"))
		i=clng(request.querystring("i"))
		if ii<>0 then i=ii-1
		...
		response.write("-----------<br/>" &ubbcode(content)& "")
		if 0<=i<PageAll then
			Response.Write "<br/>"
		end if
		...
		End if
		if cint(i)>0 then
			Response.Write i&"&nbsp;" & "<a href='smsview.asp?ids="&ids&"&amp;id=" &  rs("id") & "&amp;i=" & i-1 & "&amp;p=" & p & "&amp;TP="&TP&"&amp;sid="&sid&"'>上页</a>"
