...
end if%>标题: <%=ubb(rs("title"))%><br/>
添加日期: <%=rs("HU_date")%><br/>
人气: <%=rs("hit")%><br/>
标题:<input name="title<%=minute(now)%><%=second(now)%>" value="<%=ubbedit(rs("title"))%>"/><br/>
文章内容:<input name="test<%=minute(now)%><%=second(now)%>" value="<%=ubbedit(rs("test"))%>"/><br/>
